...
4. Wrap main content in a parent Container to manage spacing (e.g. "p-6 flex flex-col gap-6 bg-slate-50 min-h-screen").`;
            ...
            // Send messages history for multi-turn conversation context
            const response = await fetch('/api/ai/chat', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    prompt,
                    messages: messages, // Send history for multi-turn context
                }),
            });
            ...
            if (firstBracket !== -1 || firstBrace !== -1) {
                try {
                    ...
                    if (isArray) {
                        ...
                        const parsedComponents = sanitizeComponents(JSON.parse(jsonStr));
                        if (parsedComponents.length > 0 && onUpdatePage) {
                        ...
                        if (parsedData?.intent === 'create_reusable' && parsedData.component && validateComponent(parsedData.component) && onAddCustomComponent) {
                            ...
                            return;
                        } else if (parsedData?.intent === 'update_reusable' && parsedData.component && validateComponent(parsedData.component) && onUpdateCustomComponent) {
                            ...
                            return;
                        } else if (parsedData?.id && validateComponent(parsedData) && onUpdatePage) {
    ...
    return (
        <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end">
                    ...
                    >
                        ...
                        >
                            ...
                            {messages.map((m, i) => (
                                <div
                                    ...
                                    role="article"
                                    aria-label={m.role === 'user' ? '用户消息' : 'AI 助手消息'}
                                >
                            ...
                            {isLoading && (
                                <div className="flex gap-2" role="status" aria-label="AI 正在思考">
                                    ...
                                    <div className="px-3 py-2 rounded-xl text-xs bg-white border border-slate-200 text-slate-400 flex items-center gap-1">
                                        ...
                                        <span className="ml-2 text-slate-400">{t('aiCopilot.thinking')}</span>
                                    </div>
                                </div>
                            )}
                        ...
                        <div className="p-3 bg-white border-t border-slate-100 flex gap-2">
                            <input
                                ...
                                className="flex-1 text-xs px-3 py-2 bg-slate-100 rounded-lg outline-none focus:ring-2 focus:ring-primary-500/50 transition-shadow"
                                aria-label={t('aiCopilot.inputPlaceholder')}
                                disabled={isLoading}
                            />
                            <button
                                onClick={handleSend}
                                disabled={isLoading || !input.trim()}
                                className="p-2 bg-primary-600 text-white rounded-lg hover:bg-primary-700 disabled:opacity-50 transition-colors focus:ring-2 focus:ring-primary-500 focus:ring-offset-2"
            ...
            {/* Floating toggle button with accessibility */}
            <button
                onClick={() => setIsOpen(!isOpen)}
                className={`w-12 h-12 rounded-full shadow-xl flex items-center justify-center transition-all focus:ring-2 focus:ring-primary-500 focus:ring-offset-2 ${isOpen ? 'bg-slate-800 hover:bg-slate-900 text-white' : 'bg-gradient-to-r from-primary-600 to-indigo-600 hover:shadow-primary-500/50 text-white hover:scale-105'}`}
